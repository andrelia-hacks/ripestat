...
use ripestat_common::SearchcompleteRequest;

pub async fn searchcomplete(request: SearchcompleteRequest, client: &client::Client) -> Result<RipeStatResponse, RipestatClientError> {
    let data_type = RipeStatDataType::Searchcomplete;
    ...
    if let Some(v) = request.limit {
        param_vec.push(("limit".to_owned(), v.to_string()));
    }
