...
use ripestat_common::RoutingHistoryRequest;

pub async fn routing_history(request: RoutingHistoryRequest, client: &client::Client) -> Result<RipeStatResponse, RipestatClientError> {
    let data_type = RipeStatDataType::RoutingHistory;
    ...
    if let Some(v) = request.include_first_hop {
        param_vec.push(("include_first_hop".to_owned(), v.to_string()));
    }
    if let Some(v) = request.normalise_visibility {
        param_vec.push(("normalise_visibility".to_owned(), v.to_string()));
    }
    if let Some(v) = request.min_peers {
        param_vec.push(("min_peers".to_owned(), v.to_string()));
    }
