...
use ripestat_common::CountryResourceStatsRequest;

pub async fn country_resource_stats(request: CountryResourceStatsRequest, client: &client::Client) -> Result<RipeStatResponse, RipestatClientError> {
    let data_type = RipeStatDataType::CountryResourceStats;
    ...
    if let Some(v) = request.resolution {
        param_vec.push(("resolution".to_owned(), v.to_string()));
    }
