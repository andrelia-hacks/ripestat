...
use ripestat_common::RisPrefixesRequest;

pub async fn ris_prefixes(request: RisPrefixesRequest, client: &client::Client) -> Result<RipeStatResponse, RipestatClientError> {
    let data_type = RipeStatDataType::RisPrefixes;
    ...
    if let Some(v) = request.list_prefixes {
        param_vec.push(("list_prefixes".to_owned(), v.to_string()));
    }
    if let Some(v) = request.types {
        param_vec.push(("types".to_owned(), v));
    }
    if let Some(v) = request.af {
        param_vec.push(("af".to_owned(), v));
    }
    if let Some(v) = request.noise {
        param_vec.push(("noise".to_owned(), v));
    }
